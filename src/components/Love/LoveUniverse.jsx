...
const ROMANTIC_PHRASES = [
  ...
  // 🌌 Para la atmósfera de la galaxia
...
export default function LoveUniverse({ onBack }) {
  ...
  useEffect(() => {
    ...
    function updateCamera() {
      const safePhi = THREE.MathUtils.clamp(
        phi,
        0.2,
        Math.PI - 0.2
      );
      ...
      smoothMouseX +=
        (mouseX - smoothMouseX) * 0.025;

      smoothMouseY +=
        (mouseY - smoothMouseY) * 0.025;
    ...
    for (
      ...
      heartPositions[i * 3 + 1] =
        y + 18;

      heartPositions[i * 3 + 2] =
        depth;
    ...
    for (
      ...
      outlinePositions[i * 3] =
        hx * HEART_SCALE;

      outlinePositions[i * 3 + 1] =
        hy * HEART_SCALE + 18;

      outlinePositions[i * 3 + 2] =
        (Math.random() - 0.5) *
        17;
    }
    ...
    const phraseQueue = [
      ...CORE_PHRASES,
      ...ROMANTIC_PHRASES.filter(
        (p) =>
          !CORE_PHRASES.includes(p)
      ),
    ];

    let phraseIndex = 0;

    RINGS_CONFIG.forEach(
      (ring, ringIndex) => {
        for (
          let i = 0;
          i < ring.count;
    ...
    /*
     * Every active finger/pointer gets its own entry.
     *
     * 1 pointer = orbit rotation
     * 2 pointers = pinch zoom
     *
     * This avoids conflicts between touch events
     * and pointer events on touchscreen laptops.
     */
    ...
    const onPointerUpWrap = (
       ...
       * pinch before removing the pointer.
       */
      const wasPinching =
        activePointers.size >= 2;
       ...
       * ONE POINTER REMAINS
       *
       * Re-sync coordinates so rotation
       * continues without a camera jump.
       */
      ...
      if (
        ...
        /*
         * Don't interpret the end of a pinch
         * as a double tap.
         */
    ...
    const onPointerCancelWrap = (
      ...
      /*
       * Reset only when all pointers are gone.
       */
    ...
    window.__loveUniverseResetView =
      resetView;
    ...
    const animate = () => {
      ...
      const localAttribute =
        localStars.geometry.attributes
          .position;
      ...
      localAttribute.needsUpdate =
        true;

      localMaterial.opacity =
        0.50 +
        Math.sin(
          time * 1.5
        ) *
      ...
      const sparkAttribute =
        sparks.geometry.attributes
          .position;
      ...
      orbitItems.forEach(
        (item) => {
          ...
          const baseScale =
            item.sprite.userData
              .baseScale ||
            item.sprite.scale.clone();

          item.sprite.userData.baseScale =
            baseScale;
      ...
      comets.forEach(
        (comet) => {
          ...
          comet.sprite.material.opacity =
            fade * 0.42;

          const cometScale =
            17 + fade * 13;

          comet.sprite.scale.set(
            cometScale,
            cometScale,
            1
          );
      ...
      if (burstLife > 0) {
        ...
        const burstAttribute =
          burst.geometry.attributes
            .position;
        ...
        <div className="love-universe-stage">
          ...
          <style>{`
            ...
            .canvas-wrap {
              ...
              /*
               * Very important for touchscreen laptops.
               * Browser gestures are disabled so our
               * pointer-based pinch handling receives
               * both fingers consistently.
               */
              touch-action: none;
              ...
              /*
               * Prevent browser overscroll/navigation
               * gestures from interfering with the galaxy.
               */
            ...
            .galaxy-flash {
              ...
              background: radial-gradient(
                circle at center,
                rgba(255,170,220,0.22),
                rgba(255,50,145,0) 42%
              );
              opacity: 0;
          ...
          {showHint && (
            <div className="galaxy-hint">
              Arrastrar: girar · Dos dedos:
              acercar/alejar · Doble toque: flash
            </div>
          )}
        </div>
      )}
